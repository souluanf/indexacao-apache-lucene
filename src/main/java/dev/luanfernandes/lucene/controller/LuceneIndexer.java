...
public class LuceneIndexer {
    ...
    public void index(Directory index) throws IOException {
        ...
        for (int i = 0; i < 5; i++) {
            String fileName = String.format("/data/shopping-list-%s.json", i + 1);
    ...
    private Document toDocument(ShoppingList shoppingList) {
        ...
        document.add(new StringField(DocumentFields.DATE_FIELD, DateTools.dateToString(toDate(shoppingList.getDate()),
                DateTools.Resolution.DAY), Field.Store.YES));
        shoppingList.getItems().forEach(item -> document.add(new StringField(DocumentFields.ITEM_FIELD, item,
                Field.Store.YES)));
